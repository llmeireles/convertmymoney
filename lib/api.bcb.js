...
const getCotacaoAPI = url => axios.get(url) // fim de semana não retorna cotação
...
const getCotacao = dependencies => async() =>{
    //destruct assignment - recurso do JS
    const {getToday, getUrl, getCotacaoAPI, extractCotacao} = dependencies
    try{
...
module.exports = {
    getCotacaoAPI,
    extractCotacao,
    getCotacao: getCotacao({getToday, getUrl, getCotacaoAPI, extractCotacao}),//com injeção de dependência
    getToday,
    getUrl,
    pure:{
        getCotacao
